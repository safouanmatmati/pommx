...
namespace Pommx\Bridge\ApiPlatform\GraphQl\Resolver\Factory;
...
final class CollectionResolverFactory implements ResolverFactoryInterface
{
    ...
    public function __invoke(string $resourceClass = null, string $rootClass = null, string $operationName = null): callable
    {
        return function ($source, $args, $context, ResolveInfo $info) use ($resourceClass, $rootClass) {
            ...
            if (isset($source[$rootProperty = $info->fieldName], $source[ItemNormalizer::ITEM_KEY])) {
                $rootResolvedFields = $this->identifiersExtractor->getPrimaryKeyFromItem(unserialize($source[ItemNormalizer::ITEM_KEY]));
            ...
            ////////////// Start Custom Part //////////////
            //  Allow use of custom cursor - find a cleaner way instead copy/past whole bundle file...
            if ($collection instanceof PartialCursorPaginatorInterface) {
                $data = ['edges' => [], 'pageInfo' => ['endCursor' => null, 'hasNextPage' => false]];
                if ($collection instanceof CursorPaginatorInterface) {
                    $data['pageInfo']['endCursor'] = $collection->getLastCursor();
                    $data['pageInfo']['hasNextPage'] = $collection->hasNextPage();
                }

                foreach ($collection as $index => $object) {
                    $data['edges'][$index] = [
                      'node' => $this->normalizer->normalize($object, ItemNormalizer::FORMAT, $dataProviderContext),
                      'cursor' => $collection->getCursor($index)
                    ];
                }
    ...
    private function getSubresource(string $rootClass, array $rootResolvedFields, array $rootIdentifiers, string $rootProperty, string $subresourceClass, bool $isCollection, array $normalizationContext)
    {
        ...
        return $this->subresourceDataProvider->getSubresource(
            $subresourceClass, $identifiers, $normalizationContext + [
